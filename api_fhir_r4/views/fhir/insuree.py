...
class InsureeViewSet(
    ...
    def list(self, request, *args, **kwargs):
        ...
        if identifier:
            ...
            queryset = queryset.filter(validity_to__isnull=True).order_by(
                "validity_from"
            )
            ...
            if claim_date is not None:
                try:
                    claim_parse_dated = datetime.datetime.strptime(
                        claim_date, "%Y-%m-%d"
                    ).date()
                except ValueError:
                ...
                has_claim_in_range = (
                    Claim.objects.filter(date_claimed__gte=claim_parse_dated)
                    .filter(insuree_id=OuterRef("id"))
                    .values("id")
                )
                queryset = queryset.annotate(
                    has_claim_in_range=Exists(has_claim_in_range)
    ...
    def get_queryset(self):
        queryset = (
            Insuree.get_queryset(None, self.request.user)
