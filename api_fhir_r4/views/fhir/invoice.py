...
from api_fhir_r4.permissions import FHIRApiInvoicePermissions, FHIRApiBillPermissions
...
class InvoiceViewSet(
    ...
    def serializers(self):
        return {
            InvoiceSerializer: (
                self._invoice_queryset(),
                self._invoice_serializer_validator,
                (FHIRApiInvoicePermissions,),
            ),
            BillSerializer: (
                self._bill_queryset(),
                self._bill_serializer_validator,
    ...
    def _invoice_serializer_validator(cls, context):
        return cls._base_request_validator_dispatcher(
            ...
            post_check=lambda x: cls._get_type_from_body(x)
            in [item["code"] for item in InvoiceTypeMapping.invoice_type],
            put_check=lambda x: cls._get_type_from_body(x)
            in [item["code"] for item in InvoiceTypeMapping.invoice_type],
        )
    ...
    def _bill_serializer_validator(cls, context):
        return cls._base_request_validator_dispatcher(
            ...
            post_check=lambda x: cls._get_type_from_body(x)
            in [item["code"] for item in BillTypeMapping.invoice_type],
            put_check=lambda x: cls._get_type_from_body(x)
            in [item["code"] for item in BillTypeMapping.invoice_type],
        )
