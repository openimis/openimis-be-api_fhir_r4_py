...
class OrganisationViewSet(
    ...
    def serializers(self):
        return {
            HealthFacilityOrganisationSerializer: (
                self._hf_queryset(),
                self._hf_serializer_validator,
                (FHIRApiHealthServicePermissions,),
            ),
            PolicyHolderOrganisationSerializer: (
                self._ph_queryset(),
                self._ph_serializer_validator,
                (FHIRApiOrganizationPermissions,),
            ),
            InsuranceOrganizationSerializer: (
                self._io_queryset(),
                self._io_serializer_validator,
                (FHIRApiInsuranceOrganizationPermissions,),
            ),
        }
    ...
    @classmethod
    def _base_request_validator_dispatcher(
        cls, request: Request, get_check, post_check, put_check
    ):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if resource_type is None or resource_type == "ins":
            if (
                ModuleConfiguration,
                InsuranceOrganizationSerializer,
            ) in filtered_querysets:
                if (
                    len(
                        filtered_querysets[
                            ModuleConfiguration, InsuranceOrganizationSerializer
                        ]
                    )
                    == 0
                ):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        for serializer, (qs, _, _) in self.get_eligible_serializers_iterator():
            if qs.model is not ModuleConfiguration:
                ...
                if qs.count() > 0:
                    data = self._get_insurance_organisation(
                        kwargs.get("identifier", None)
                    )
                else:
                    data = self._get_insurance_organisation_default(
                        kwargs.get("identifier", None)
                    )
                if data:
        ...
        if len(retrieved) > 1:
            ...
            if len(errors) > 0:
                raise Http404(
                    f"Resource for identifier {kwargs['identifier']} cannot be reloved:{','.join(errors)}"
                )
            else:
                raise Http404(
                    f"Resource for identifier {kwargs['identifier']} not found"
                )
        ...
        for model, model_data in data.items():
            ...
            if not serializer_cls:
                if all(isinstance(md, OrderedDict) for md in model_data):
                    ...
                    data_default = self.__check_default_config_insurance_organisation(
    ...
    def __check_default_config_insurance_organisation(self, model_data, user=None):
        for md in model_data:
            if md.get("resource_type") == "insurance_organisation":
                ...
                return [
                    self._get_insurance_organisation_default(
