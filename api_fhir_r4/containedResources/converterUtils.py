...
def get_converted_contained_resource(
    ...
    if reference:
        ...
        contained = [
            n
            for n in contained
            if n.resource_type == resource_type and n.id == resource_id
        ]

        _validate_matching_contained(contained)
        if len(contained) > 0:
            return converter_for_contained.to_imis_obj(
                contained[0].__dict__, audit_user_id
            )

    return None


def get_from_contained_or_by_reference(
    fhir_reference, contained, converter, audit_user_id
):
    """
