...
class HealthFacilityOrganizationTypeMapping:
    ...
    LEVEL_DISPLAY_MAPPING = {"D": "Dispensary", "C": "Health Centre", "H": "Hospital"}

    LEVEL_SYSTEM = (
        f"{GeneralConfiguration.get_system_base_url()}/CodeSystem/organization-hf-level"
    )

    TYPE_SYSTEM = f"{GeneralConfiguration.get_system_base_url()}/CodeSystem/organization-hf-care-type"

    TYPE_DISPLAY_MAPPING = {"O": "Out-patient", "I": "In-patient", "B": "Both"}
...
class PolicyHolderConfigMapping:
    ...
    def fhir_ph_code_system(cls, code):
        ...
        def __code_system_dict():
            system = urljoin(
                GeneralConfiguration.get_system_base_url(), cls._get_system()
            )
            dict_ = {}
