...
class RelationshipMapping(object):
    ...
    def load(cls):
        if cls.relationship == {}:
            cls.relationship = {
                str(relation.id): relation.relation
...
class EducationLevelMapping(object):
    ...
    def load(cls):
        if cls.education_level == {}:
            cls.education_level = {
                str(education.id): education.education
    ...
    def load(cls):
        if cls.patient_profession == {}:
            cls.patient_profession = {
                str(profession.id): profession.profession
    ...
    def load(cls):
        if cls.identification_type == {}:
            cls.identification_type = {
                identification.code: identification.identification_type
                for identification in IdentificationType.objects.all()
            }
...
class PatientCategoryMapping(object):
    ...
    AGE_SYSTEM = (
        f"{GeneralConfiguration.get_system_base_url()}CodeSystem/usage-context-age-type"
    )
    imis_gender_mapping = {}
