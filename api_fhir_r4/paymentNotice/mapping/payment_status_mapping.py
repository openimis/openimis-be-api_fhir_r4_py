...
class PaymentNoticePaymentStatusMapping:
    _IMIS_RECONCILIATED_CLEARED = (
        PaymentInvoice.ReconciliationStatus.RECONCILIATED.value
    )
    _FHIR_RECONCILIATED_CLEARED = (
        R4PaymentNoticeConfig.get_fhir_payment_notice_payment_status_cleared()
    )

    _IMIS_NOT_RECONCILIATED_PAID = (
        PaymentInvoice.ReconciliationStatus.NOT_RECONCILIATED.value
    )
    _FHIR_NOT_RECONCILIATED_PAID = (
        R4PaymentNoticeConfig.get_fhir_payment_notice_payment_status_paid()
    )
