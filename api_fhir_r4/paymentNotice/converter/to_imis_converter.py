...
from .errors import ERROR_BAD_STATUS, ERROR_BAD_PAYMENT_STATUS, ERROR_BAD_TYPE_REQUEST
...
class PaymentNoticeToImisConverter(BaseFHIRConverter, ReferenceConverterMixin):
    ...
    def to_imis_obj(cls, fhir_payment_notice, audit_user_id=None):
        ...
        cls.build_imis_payment_date_created(
            imis_payment, imis_payment_detail, fhir_payment_notice
        )
        cls.build_imis_payment_payer_ref(imis_payment, fhir_payment_notice)
        cls.build_imis_payment_reconciliation_status(
            imis_payment, fhir_payment_notice, errors
        )
        cls.build_imis_payment_amount(
            imis_payment, imis_payment_detail, fhir_payment_notice
        )
        ...
        cls.build_imis_payment_detail_invoice(
            imis_payment_detail, fhir_payment_notice, errors
        )
        cls.build_imis_payment_detail_status(
            imis_payment_detail, fhir_payment_notice, errors
        )
        invoice_status = None
    ...
    @classmethod
    def build_imis_payment_date_created(
        cls, imis_payment, imis_payment_detail, fhir_payment_notice
    ):
    ...
    @classmethod
    def build_imis_payment_reconciliation_status(
        cls, imis_payment, fhir_payment_notice, errors
    ):
        coding = cls.get_first_coding_from_codeable_concept(
            fhir_payment_notice.paymentStatus
        )
        payment_status = coding.code
        if payment_status in PAYMENT_STATUS:
            imis_payment.reconciliation_status = (
                PaymentNoticePaymentStatusMapping.to_imis_status[payment_status]
            )
        else:
    ...
    def get_imis_invoice_status(cls, fhir_payment_notice, errors):
        ...
        if status in STATUS:
            invoice_status = PaymentNoticeStatusMapping.to_imis_status[status]
    ...
    @classmethod
    def build_imis_payment_detail_status(
        cls, imis_payment_detail, fhir_payment_notice, errors
    ):
        if fhir_payment_notice.status in STATUS:
            imis_payment_detail.status = (
                PaymentNoticePaymentDetailStatusMapping.to_imis_status[
                    fhir_payment_notice.status
                ]
            )
        else:
            errors.append(ERROR_BAD_STATUS)
    ...
    @classmethod
    def build_imis_payment_detail_invoice(
        cls, imis_payment_detail, fhir_payment_notice, errors
    ):
