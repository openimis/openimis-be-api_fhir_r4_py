...
from api_fhir_r4.multiserializer.mixins import (
    MultiSerializerUpdateModelMixin,
    MultiSerializerRetrieveModelMixin,
)
...
class UpdateModelMixin(RestUpdateModelMixin):
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(
            instance, data=request.data, partial=partial, user=request.user
        )
        serializer.is_valid(raise_exception=True)
...
class ListModelMixin(RestListModelMixin):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if not isinstance(queryset, Iterable):
            queryset = queryset.all().order_by(
                "code" if hasattr(queryset.model, "code") else "id"
            )
    ...
    def _get_object_with_first_valid_retriever(self, identifier):
        for retriever in self.retrievers:
            if retriever.identifier_validator(identifier):
                try:
                    queryset = retriever.retriever_additional_queryset_filtering(
                        self.get_queryset()
                    )
...
class MultiIdentifierRetrieverMixin(
    RetrieveModelMixin, GenericMultiIdentifierMixin, ABC
):

    def retrieve(self, request, *args, **kwargs):
        ref_type, instance = self._get_object_with_first_valid_retriever(
            kwargs["identifier"]
        )
        serializer = self.get_serializer(
            instance, reference_type=ref_type, user=request.user
        )
        return Response(serializer.data)
...
class MultiIdentifierUpdateManySerializersMixin(
    MultiSerializerUpdateModelMixin, GenericMultiIdentifierForManySerializers, ABC
):
    def update(self, request, *args, **kwargs):
...
class MultiIdentifierRetrieveManySerializersMixin(
    MultiSerializerRetrieveModelMixin, GenericMultiIdentifierForManySerializers, ABC
):
    def retrieve(self, request, *args, **kwargs):
