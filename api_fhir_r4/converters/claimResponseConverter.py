...
class ClaimResponseConverter(BaseFHIRConverter):
    ...
    def get_imis_claim_from_response(cls, fhir_claim_response):
        ...
        try:
            ...
            raise FHIRRequestProcessException(
                f"Claim Response cannot be created from scratch, "
    ...
    def build_patient_reference(cls, fhir_claim_response, imis_claim, reference_type):
        fhir_claim_response.patient = PatientConverter.build_fhir_resource_reference(
            imis_claim.insuree, reference_type=reference_type
        )
    ...
    def build_fhir_total_list(cls, fhir_claim_response, imis_claim):
            ...
            for item in items:
                for adjudication in item.adjudication:
                    ...
                    if rejection_reason == "0":
                        if adjudication.category.coding[0].code == str(
                            Claim.STATUS_ENTERED
                        ):
                            total_claimed += (
                                adjudication.amount.value * adjudication.value
                            )
                        if adjudication.category.coding[0].code == str(
        ...
        if imis_claim.status >= Claim.STATUS_ENTERED:
            fhir_total.append(
                cls.build_fhir_total(imis_claim, Claim.STATUS_ENTERED, total_claimed)
            )
        if imis_claim.status >= Claim.STATUS_CHECKED:
            fhir_total.append(
                cls.build_fhir_total(imis_claim, Claim.STATUS_CHECKED, total_approved)
            )
        if imis_claim.status >= Claim.STATUS_PROCESSED:
            fhir_total.append(
                cls.build_fhir_total(imis_claim, Claim.STATUS_PROCESSED, total_adjusted)
            )
        if imis_claim.status == Claim.STATUS_VALUATED:
            fhir_total.append(
                cls.build_fhir_total(imis_claim, Claim.STATUS_VALUATED, total_valuated)
            )
    ...
    @classmethod
    def build_fhir_communication_request_reference(
        cls, fhir_claim_response, imis_claim, reference_type
    ):
        try:
            if imis_claim.feedback is not None:
                request = CommunicationRequestConverter.build_fhir_resource_reference(
                    imis_claim.feedback, reference_type=reference_type
                )
    ...
    @classmethod
    def build_imis_communication_request_reference(
        cls, imis_claim, fhir_claim_response
    ):
        try:
    ...
    def build_fhir_items(cls, fhir_claim_response, imis_claim, reference_type):
        ...
        cls.build_fhir_items_for_imis_items(
            fhir_claim_response, imis_claim, reference_type
        )
        cls.build_fhir_items_for_imis_services(
            fhir_claim_response, imis_claim, reference_type
        )
    ...
    def build_imis_items(cls, imis_claim: Claim, fhir_claim_response: ClaimResponse):
        ...
        for item in fhir_claim_response.item:
            cls._build_imis_claim_item(
                imis_claim, fhir_claim_response, item
            )  # same for item and service
    ...
    def get_imis_claim_item_by_code(cls, code, imis_claim_id):
        ...
        result = ClaimItem.objects.filter(
            item_id__in=Subquery(item_code_qs.values("id")), claim_id=imis_claim_id
        )
        return result[0] if len(result) > 0 else None

    @classmethod
    def _build_imis_claim_item(
        cls, imis_claim, fhir_claim_response: ClaimResponse, item: ClaimResponseItem
    ):
        ...
        if extension.valueReference.type == "Medication":
            ...
            claim_item = ClaimService.objects.get(
                claim=imis_claim, service=imis_service
            )
        else:
            raise FHIRRequestProcessException(
                f"Unknnown serviced item type: {extension.url}"
            )
    ...
    def get_imis_claim_service_by_code(cls, code, imis_claim_id):
        ...
        result = ClaimService.objects.filter(
            service_id__in=Subquery(service_code_qs.values("id")),
    ...
    def build_fhir_item(
        ...
        claim_response_item.itemSequence = FhirUtils.get_next_array_sequential_id(
            fhir_claim_response.item
        )
        ...
        if type == "item":
            ...
            raise FHIRRequestProcessException(
                f"Unknown type of serviced product: {type}"
            )

        serviced_extension = cls.build_serviced_extension(
            serviced_item, service_type, reference_type
        )
    ...
    def build_serviced_extension(cls, serviced, service_type, reference_type):
        ...
        extension.valueReference = MedicationConverter.build_fhir_resource_reference(
            serviced, service_type, reference_type=reference_type, display=serviced.code
        )
        return extension
    ...
    @classmethod
    def __build_adjudication(
        cls, item, rejected_reason, amount, category, quantity, explicit_amount=False
    ):
    ...
    def build_fhir_item_adjudication(cls, item, rejected_reason, imis_claim):
        def build_asked_adjudication(status, price):
            ...
            adjudication = cls.__build_adjudication(
                item, rejected_reason, price, category, item.qty_provided, True
            )
            return adjudication
        ...
        def build_processed_adjudication(status, price):
            ...
            adjudication = cls.__build_adjudication(
                item, rejected_reason, price, category, quantity
            )
            return adjudication
        ...
        if rejected_reason == 0 and imis_claim.status != 1:
            if imis_claim.status >= Claim.STATUS_ENTERED:
                adjudications.append(
                    build_asked_adjudication(Claim.STATUS_ENTERED, price_asked)
                )
            ...
            if imis_claim.status >= Claim.STATUS_CHECKED:
                if item.price_approved:
                    ...
                    adjudications.append(
                        build_processed_adjudication(
                            Claim.STATUS_CHECKED, price_approved
                        )
                    )
                else:
                    adjudications.append(
                        build_processed_adjudication(Claim.STATUS_CHECKED, price_asked)
                    )
            if imis_claim.status >= Claim.STATUS_PROCESSED:
                if item.price_adjusted:
                    ...
                    adjudications.append(
                        build_processed_adjudication(
                            Claim.STATUS_PROCESSED, price_adjusted
                        )
                    )
                else:
                    adjudications.append(
                        build_processed_adjudication(
                            Claim.STATUS_PROCESSED, price_asked
                        )
                    )
            if imis_claim.status == Claim.STATUS_VALUATED:
                if item.price_valuated:
                    ...
                    adjudications.append(
                        build_processed_adjudication(
                            Claim.STATUS_VALUATED, price_valuated
                        )
                    )
                else:
                    adjudications.append(
                        build_processed_adjudication(Claim.STATUS_VALUATED, price_asked)
                    )
        else:
    ...
    def build_item_valuated(cls, claim_item, adjudication):
        ...
        if (
            adjudication.amount
            and adjudication.amount.value
            != claim_item.price_asked * claim_item.qty_provided
        ):
    ...
    def build_fhir_requestor(cls, fhir_claim_response, imis_claim, reference_type):
        if imis_claim.admin is not None:
            fhir_claim_response.requestor = (
                ClaimAdminPractitionerConverter.build_fhir_resource_reference(
                    imis_claim.admin, reference_type=reference_type
                )
            )
