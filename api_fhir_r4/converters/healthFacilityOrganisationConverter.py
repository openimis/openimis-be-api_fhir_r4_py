...
from api_fhir_r4.mapping.organizationMapping import (
    HealthFacilityOrganizationTypeMapping,
)
...
class HealthFacilityOrganisationConverter(
    ...
    def to_fhir_obj(
        cls,
        imis_organisation: HealthFacility,
    ...
    @classmethod
    def build_fhir_extensions(
        cls, fhir_organisation: Organization, imis_organisation: HealthFacility
    ):
        extensions = [
    ...
    def build_fhir_type(cls, fhir_organisation, imis_organisation):
        organisation_type = cls.build_codeable_concept(
            code=HealthFacilityOrganizationTypeMapping.ORGANIZATION_TYPE
        )
    ...
    def _build_address_ext(cls, imis_organisation, reference_type):
        address_ref = LocationConverter.build_fhir_resource_reference(
            imis_organisation, "Organization", reference_type=reference_type
        )
        address_ext = Extension.construct()
        address_ext.url = (
            HealthFacilityOrganizationTypeMapping.ADDRESS_LOCATION_REFERENCE_URL
        )
    ...
    def build_contacts(cls, fhir_organisation, imis_organisation):
        contracts = []
        relevant_claim_admins = ClaimAdmin.objects.filter(
            health_facility=imis_organisation, validity_to__isnull=True
        ).distinct()

        for admin in relevant_claim_admins:
            contracts.append(cls._build_claim_admin_contract(admin))
    ...
    def _build_claim_admin_contract(cls, admin):
        ...
        contract.purpose = cls.build_codeable_concept(
            **HealthFacilityOrganizationTypeMapping.CONTRACT_PURPOSE
        )
    ...
    def __legal_form_extension(cls, imis_organisation: HealthFacility):
        if imis_organisation.legal_form:
            ...
            extension.valueCodeableConcept = cls.build_codeable_concept(
                ...
                display=HealthFacilityOrganizationTypeMapping.LEGAL_FORM_MAPPING.get(
                    legal_form
                ),
            )
            return extension
    ...
    def __level_extension(cls, imis_organisation):
        ...
        level_display = HealthFacilityOrganizationTypeMapping.LEVEL_DISPLAY_MAPPING.get(
            level, None
        )
    ...
    def __type_extension(cls, imis_organisation: HealthFacility):
        if imis_organisation.care_type and imis_organisation.care_type != " ":
            ...
            care_type_display = (
                HealthFacilityOrganizationTypeMapping.TYPE_DISPLAY_MAPPING.get(
                    care_type, None
                )
            )
    ...
    def build_imis_hf_identiftier(cls, imis_hf, fhir_hf, errors):
        value = cls.get_fhir_identifier_by_code(
            fhir_hf.identifier, R4IdentifierConfig.get_fhir_generic_type_code()
        )

        if value:
            imis_hf.code = value
        cls.valid_condition(
            imis_hf.code is None, _("Missing HF Organization code"), errors
        )
    ...
    def __get_unique_telecom(cls, fhir_hf, system, errors):
        ...
        cls.valid_condition(
            len(telecom) > 1,
            _(
                f"More than one contact point of system {system} assigned to health facility"
            ),
            errors,
        )
    ...
    @classmethod
    def build_imis_hf_address(
        cls, imis_hf: HealthFacility, fhir_hf: Organization, errors
    ):
        if fhir_hf.address:
    ...
    def build_imis_hf_level(cls, imis_hf, fhir_hf, errors):
        ...
        imis_hf.level = cls.__get_extension_by_url_suffix(
            fhir_hf.extension, ext_url_suffix
        )
    ...
    def build_imis_care_type(cls, imis_hf, fhir_hf, errors):
        ...
        if value:
            ...
            cls.valid_condition(
                True, _("Extension with HF care type not found"), errors
            )
    ...
    def build_imis_legal_form(cls, imis_hf, fhir_hf, errors):
        ...
        cls.valid_condition(
            value
            not in HealthFacilityOrganizationTypeMapping.LEGAL_FORM_MAPPING.keys(),
        ...
        if value:
            ...
            cls.valid_condition(
                True, _("Extension with HF legal form not found"), errors
            )

    @classmethod
    def __get_extension_by_url_suffix(cls, extensions, ext_url_suffix):
        level_ext = next(
            (x for x in extensions if x.url.endswith(ext_url_suffix)), None
        )
        if level_ext:
            return cls.get_first_coding_from_codeable_concept(
                level_ext.valueCodeableConcept
            ).code
        else:
            return None
    ...
    def build_imis_parent_location_id(cls, imis_hf, fhir_hf, errors):
        if not fhir_hf.address or len(fhir_hf.address) == 0:
            ...
            return
        imis_hf.location = LocationConverter.get_location_from_address(
            LocationConverter, fhir_hf.address[0]
        )
