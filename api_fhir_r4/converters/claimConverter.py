...
from api_fhir_r4.containedResources.converterUtils import (
    get_from_contained_or_by_reference,
)
...
from api_fhir_r4.converters.claimAdminPractitionerConverter import (
    ClaimAdminPractitionerConverter,
)
...
class ClaimConverter(BaseFHIRConverter, ReferenceConverterMixin):
    ...
    def to_imis_obj(cls, fhir_claim, audit_user_id):
        ...
        cls.build_imis_health_facility(
            errors, fhir_claim, imis_claim, audit_user_id=audit_user_id
        )
        cls.build_imis_identifier(imis_claim, fhir_claim, errors)
        cls.build_imis_patient(
            imis_claim, fhir_claim, errors, audit_user_id=audit_user_id
        )
    ...
    def get_imis_obj_by_fhir_reference(cls, reference, errors=None):
        return DbManagerUtils.get_object_or_none(
            Claim, **cls.get_database_query_id_parameteres_from_reference(reference)
        )
    ...
    def build_imis_date_claimed(cls, imis_claim, fhir_claim, errors):
        ...
        cls.valid_condition(
            not imis_claim.date_claimed, _("Missing `created` attribute"), errors
        )
    ...
    def build_imis_identifier(cls, imis_claim, fhir_claim, errors):
        value = cls.get_fhir_identifier_by_code(
            fhir_claim.identifier, cls.get_fhir_code_identifier_type()
        )
        if value:
            imis_claim.code = value
        cls.valid_condition(
            not imis_claim.code, _("Missing or invalid `identifier` attribute"), errors
        )
    ...
    def build_imis_patient(cls, imis_claim, fhir_claim, errors, audit_user_id):
        ...
        cls.valid_condition(
            not imis_claim.insuree, _("Missing or invalid `patient` reference"), errors
        )
    ...
    def build_imis_health_facility(cls, errors, fhir_claim, imis_claim, audit_user_id):
        ...
        cls.valid_condition(
            not imis_claim.health_facility,
            _("Missing or invalid `provider` reference"),
            errors,
        )
    ...
    def build_imis_date_range(cls, imis_claim, fhir_claim, errors):
        ...
        cls.valid_condition(
            not imis_claim.date_from,
            _("Missing or invalid 'billable_period' attribute"),
            errors,
        )
    ...
    def build_fhir_diagnosis(cls, diagnoses, icd):
        ...
        diagnosis_codeable_concept = cls.build_codeable_concept(
            icd.code, system=system, display=icd.name
        )
        claim_diagnosis_data = {
            "sequence": FhirUtils.get_next_array_sequential_id(diagnoses),
    ...
    def build_imis_diagnoses(cls, imis_claim, fhir_claim, errors):
        ...
        cls.valid_condition(
            imis_claim.icd is None,
            _("Missing or invalid `diagnosis` attribute"),
            errors,
        )

    @classmethod
    def get_imis_diagnosis_by_code(cls, icd_code):
        return Diagnosis.objects.get(code=icd_code, *Diagnosis.filter_validity())
    ...
    def build_imis_total_claimed(cls, imis_claim, fhir_claim, errors):
        ...
        cls.valid_condition(
            not imis_claim.claimed, _("Missing `total` attribute"), errors
        )
    ...
    def build_imis_claim_admin(cls, imis_claim, fhir_claim, errors, audit_user_id):
        ...
        cls.valid_condition(
            imis_claim.admin is None,
            _("Missing or invalid `enterer` reference"),
            errors,
        )
    ...
    def build_imis_visit_type(cls, imis_claim, fhir_claim, errors):
        ...
        cls.valid_condition(
            not imis_claim.visit_type, _("Missing or invalid `type` attribute"), errors
        )
    ...
    def build_fhir_supporting_info(cls, fhir_claim, imis_claim):
        ...
        cls.build_fhir_string_information(
            supporting_info, guarantee_id_code, imis_claim.guarantee_id
        )
        explanation_code = R4ClaimConfig.get_fhir_claim_information_explanation_code()
        cls.build_fhir_string_information(
            supporting_info, explanation_code, imis_claim.explanation
        )
    ...
    def build_imis_supporting_info(cls, imis_claim, fhir_claim, errors):
        if (
            not hasattr(imis_claim, "claim_attachments")
            or imis_claim.claim_attachments is not list
        ):
            imis_claim.claim_attachments = []
        if fhir_claim.supportingInfo:
            for supporting_info in fhir_claim.supportingInfo:
                category_coding = cls.get_first_coding_from_codeable_concept(
                    supporting_info.category
                )
                category = category_coding.code
                if (
                    category
                    == R4ClaimConfig.get_fhir_claim_information_guarantee_id_code()
                ):
                    imis_claim.guarantee_id = supporting_info.valueString
                elif (
                    category
                    == R4ClaimConfig.get_fhir_claim_information_explanation_code()
                ):
                    imis_claim.explanation = supporting_info.valueString
                elif category == R4ClaimConfig.get_fhir_claim_attachment_code():
                    claim_attachment = cls.build_attachment_from_value(
                        supporting_info.valueAttachment
                    )
                    imis_claim.claim_attachments.append(claim_attachment)
                else:
                    cls.valid_condition(
                        True,
                        _("Unknown supporting info category: `%s`") % category,
                        errors,
                    )
    ...
    def build_fhir_string_information(cls, supporting_info, code, value_string):
        if value_string:
            ...
            supporting_info_entry.sequence = FhirUtils.get_next_array_sequential_id(
                supporting_info
            )
            base = GeneralConfiguration.get_system_base_url()
            system = urljoin(
                base, R4ClaimConfig.get_fhir_claim_supporting_info_system()
            )
    ...
    def build_medication_extension(cls, item, reference_type):
        ...
        reference = cls.build_fhir_resource_reference(
            item.item, type="Medication", reference_type=reference_type
        )
    ...
    def build_activity_definition_extension(cls, service, reference_type):
        ...
        url = urljoin(base, R4ClaimConfig.get_fhir_item_reference_extension_system())
        reference = cls.build_fhir_resource_reference(
            service.service, type="ActivityDefinition", reference_type=reference_type
        )
        return cls.build_fhir_reference_extension(reference, url)

    @classmethod
    def build_imis_submit_items_and_services(
        cls, imis_claim, fhir_claim, errors, audit_user_id
    ):
        imis_items = []
        imis_services = []
        if fhir_claim.item:
            for item in fhir_claim.item:
                ...
                if category == R4ClaimConfig.get_fhir_claim_item_code():
                    cls.build_imis_submit_item(
                        imis_items, item, fhir_claim, audit_user_id
                    )
                elif category == R4ClaimConfig.get_fhir_claim_service_code():
                    cls.build_imis_submit_service(
                        imis_services, item, fhir_claim, audit_user_id
                    )
                else:
                    cls.valid_condition(
                        True, _("Unknown item category: `%s`") % category, errors
                    )
    ...
    def build_imis_submit_item(cls, imis_items, fhir_item, fhir_claim, audit_user_id):
        item = cls.__get_provision_from_contained_or_reference(
            fhir_item, fhir_claim, MedicationConverter, audit_user_id
        )
    ...
    @classmethod
    def build_imis_submit_service(
        cls, imis_services, fhir_item, fhir_claim, audit_user_id
    ):
        claim_service = ClaimService(
            qty_provided=cls.get_fhir_item_qty_provided(fhir_item),
            price_asked=cls.get_fhir_item_price_asked(fhir_item),
            service=cls.__get_provision_from_contained_or_reference(
                fhir_item, fhir_claim, ActivityDefinitionConverter, audit_user_id
            ),
        )
    ...
    @classmethod
    def __get_provision_from_contained_or_reference(
        cls, fhir_provision, fhir_claim, converter, audit_user_id
    ):
    ...
    @classmethod
    def _get_imis_claim_provision(
        cls, fhir_item, submission_type: Type[ClaimElementSubmit]
    ):
        price_asked = cls.get_fhir_item_price_asked(fhir_item)
    ...
    def build_fhir_priority(cls, fhir_claim):
        ...
        fhir_claim.priority = cls.build_codeable_concept_from_coding(
            cls.build_fhir_mapped_coding(mapping)
        )
    ...
    def build_fhir_insurance(cls, fhir_claim, imis_claim, reference_type):
        ...
        if policies:
            sorted_by_enrol = sorted(
                list(policies), key=lambda x: x.effective_date, reverse=True
            )
            latest = sorted_by_enrol[0]
            claim_insurance_data = {"focal": True, "sequence": 1}
            insurance = ClaimInsurance(**claim_insurance_data)
            insurance.coverage = cls.build_fhir_resource_reference(
                latest.policy, type="Coverage", reference_type=reference_type
            )
    ...
    def build_fhir_attachments(cls, fhir_claim, imis_claim):
        ...
        for attachment in attachments:
            supporting_info_element = cls.build_attachment_supporting_info_element(
                attachment
            )
    ...
    def build_attachment_supporting_info_element(cls, imis_attachment):
        ...
        supporting_info_element.category = (
            cls.build_attachment_supporting_info_category()
        )
        supporting_info_element.valueAttachment = cls.build_fhir_value_attachment(
            imis_attachment
        )
    ...
    def get_attachment_content(cls, imis_attachment):
        ...
        if file_root and imis_attachment.url:
            with open(
                "%s/%s"
                % (ClaimConfig.claim_attachments_root_path, imis_attachment.url),
                "rb",
            ) as file:
    ...
    def build_attachment_from_value(cls, valueAttachment: Attachment):
        allowed_mime_regex = (
            R4ClaimConfig.get_allowed_fhir_claim_attachment_mime_types_regex()
        )
    ...
    def build_fhir_provider(cls, fhir_claim, imis_claim, reference_type):
        fhir_claim.provider = cls.build_fhir_resource_reference(
    ...
    def build_fhir_patient(cls, fhir_claim, imis_claim, reference_type):
        fhir_claim.patient = cls.build_fhir_resource_reference(
            imis_claim.insuree,
            type="Patient",
    ...
    def build_fhir_enterer(cls, fhir_claim, imis_claim, reference_type):
        fhir_claim.enterer = cls.build_fhir_resource_reference(
            imis_claim.admin,
            type="Practitioner",
