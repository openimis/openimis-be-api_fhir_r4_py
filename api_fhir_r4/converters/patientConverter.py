...
class PatientConverter(
    BaseFHIRConverter, PersonConverterMixin, ReferenceConverterMixin
):

    @classmethod
    def to_fhir_obj(
        cls, imis_insuree, reference_type=ReferenceConverterMixin.UUID_REFERENCE_TYPE
    ):
    ...
    def build_imis_extentions(cls, imis_insuree, fhir_patient, errors):
        ...
        for extension in fhir_patient.extension:
            if (
                extension.url
                == f"{GeneralConfiguration.get_system_base_url()}StructureDefinition/patient-is-head"
            ):
                imis_insuree.head = extension.valueBoolean

            elif (
                extension.url
                == f"{GeneralConfiguration.get_system_base_url()}StructureDefinition/patient-education-level"
            ):
                try:
                    imis_insuree.education = Education.objects.get(
                        id=extension.valueCodeableConcept.coding[0].code
                    )
                except Exception:
                ...
                extension.url
                == f"{GeneralConfiguration.get_system_base_url()}StructureDefinition/patient-profession"
            ):
                try:
                    imis_insuree.profession = Profession.objects.get(
                        id=extension.valueCodeableConcept.coding[0].code
                    )
                except Exception:
                ...
                extension.url
                == f"{GeneralConfiguration.get_system_base_url()}StructureDefinition/patient-card-issued"
            ):
                try:
                ...
                extension.url
                == f"{GeneralConfiguration.get_system_base_url()}StructureDefinition/patient-identification"
            ):
                try:
                    for ext in extension.extension:
                        ...
                        if ext.url == "type":
                            imis_insuree.type_of_id = IdentificationType.objects.get(
    ...
    def createDefaultInsuree(cls, audit_user_id):
        ...
        imis_insuree.head = (
            GeneralConfiguration.get_default_value_of_patient_head_attribute()
        )
        imis_insuree.card_issued = (
            GeneralConfiguration.get_default_value_of_patient_card_issued_attribute()
        )
    ...
    def build_imis_names(cls, imis_insuree, fhir_patient, errors):
        ...
        imis_insuree.last_name, imis_insuree.other_names = (
            cls.build_imis_last_and_other_name(names)
        )
    ...
    def __build_chfid_identifier(cls, chfid):
        return cls.build_fhir_identifier(
            chfid,
    ...
    def build_imis_identifiers(cls, imis_insuree, fhir_patient):
        ...
        if chf_id := cls.get_fhir_identifier_by_code(
            insuree_ids, R4IdentifierConfig.get_fhir_generic_type_code()
        ):
            imis_insuree.chf_id = chf_id
        else:
            raise FHIRException("Patient code not provided.")

        if passport := cls.get_fhir_identifier_by_code(
            insuree_ids, R4IdentifierConfig.get_fhir_passport_type_code()
        ):
    ...
    def build_fhir_passport_identifier(cls, identifiers, imis_insuree):
        if hasattr(imis_insuree, "type_of_id") and imis_insuree.type_of_id is not None:
            pass
        # TODO typeofid isn't provided, this section should contain logic used
        # to create passport field based on typeofid
                ...
                if contact.relationship:
                    ...
                    for relationship in contact.relationship:
                        for coding in relationship.coding:
                            if (
                                "CodeSystem/patient-contact-relationship"
                                in coding.system
                            ):
    ...
    def build_imis_birth_date(cls, imis_insuree, fhir_patient, errors):
        ...
        if not cls.valid_condition(
            birth_date is None, _("Missing patient `birthDate` attribute"), errors
        ):
    ...
    def build_fhir_marital_status(cls, fhir_patient, imis_insuree):
        if imis_insuree.marital:
            ...
            fhir_patient.maritalStatus = cls.build_codeable_concept(
                code=imis_insuree.marital,
    ...
    def build_fhir_telecom(cls, fhir_patient, imis_insuree):
        fhir_patient.telecom = cls.build_fhir_telecom_for_person(
            phone=imis_insuree.phone, email=imis_insuree.email
        )

    @classmethod
    def build_imis_contacts(cls, imis_insuree, fhir_patient):
        imis_insuree.phone, imis_insuree.email = cls.build_imis_phone_num_and_email(
    ...
    def build_fhir_addresses(cls, fhir_patient, imis_insuree, reference_type):
        ...
        if imis_insuree.current_village:
            ...
            family_address = cls._build_insuree_family_address(
                imis_insuree.family, reference_type
            )
    ...
    def build_imis_addresses(cls, imis_insuree, fhir_patient):
        ...
        provided_address = cls.__get_address_by_properties(
            fhir_patient, type_="physical"
        )
    ...
    def _build_imis_current_patient_address(cls, imis_insuree, fhir_patient):
        cls._validate_fhir_address_details(fhir_patient.address)
        if insuree_address := cls.__get_insuree_current_address_from_fhir_patinet(
            fhir_patient
        ):
    ...
    def _build_or_validate_family_address(cls, imis_insuree, fhir_patient):
        if not (
            cls.__get_family_address_from_fhir_patient(
                fhir_patient
            )
        ):
            return

        if not (cls.__get_family_from_fhir_patient_extension(fhir_patient)):
        ...
        def build_extension(fhir_patient, imis_insuree, value):
            ...
            if value == "head":
                ...
                if (
                    ...
                    display = EducationLevelMapping.education_level[
                        str(imis_insuree.education.id)
                    ]
                    system = "CodeSystem/patient-education-level"
                    extension.valueCodeableConcept = cls.build_codeable_concept(
                        code=str(imis_insuree.education.id), system=system
                    )
                ...
                if hasattr(imis_insuree, "type_of_id") and imis_insuree.type_of_id:
                    if hasattr(imis_insuree, "passport") and imis_insuree.passport:
                        ...
                        nested_extension.valueCodeableConcept = (
                            cls.build_codeable_concept(
                                code=imis_insuree.type_of_id.code, system=system
                            )
                        )
                ...
                if (
                    ...
                    display = PatientProfessionMapping.patient_profession[
                        str(imis_insuree.profession.id)
                    ]
                    system = "CodeSystem/patient-profession"
                    extension.valueCodeableConcept = cls.build_codeable_concept(
                        code=str(imis_insuree.profession.id), system=system
                    )
                    if len(extension.valueCodeableConcept.coding) == 1:
                        extension.valueCodeableConcept.coding[0].display = display
    ...
    def build_fhir_contact(cls, fhir_patient, imis_insuree):
        ...
        if (
            imis_insuree.relationship is not None
            and imis_insuree.family is not None
            ...
            display = RelationshipMapping.relationship[
                str(imis_insuree.relationship.id)
            ]
            fhir_contact.relationship = [
                cls.build_codeable_concept(
                    code=imis_insuree.relationship.id, system=system
                )
            ]
    ...
    def build_fhir_photo(cls, fhir_patient, imis_insuree):
        if (
            imis_insuree.photo
            and imis_insuree.photo.folder
            and imis_insuree.photo.filename
        ):
            ...
            abs_url = (
                GeneralConfiguration.get_host_domain().split("http://")[1]
                or "localhost"
            )
            domain = abs_url
            ...
            photo.contentType = imis_insuree.photo.filename[
                imis_insuree.photo.filename.rfind(".") + 1:
            ]
    ...
    def build_imis_photo(cls, imis_insuree, fhir_patient, errors):
        if fhir_patient.photo and len(fhir_patient.photo) > 0:
            ...
            if fhir_patient.photo[0].data:
                ...
                obj, created = InsureePhoto.objects.get_or_create(
    ...
    @classmethod
    def build_fhir_general_practitioner(
        cls, fhir_patient, imis_insuree, reference_type
    ):
    ...
    def build_imis_general_practitioner(cls, imis_insuree, fhir_patient):
        ...
        if len(fhir_patient.generalPractitioner) != 1:
            ...
            raise FHIRException(
                _("Patient can provide at most one general practitioner.")
            )

        hf_uuid = cls.get_resource_id_from_reference(
            fhir_patient.generalPractitioner[0]
        )
        try:
            ...
            raise FHIRException(
                f"Invalid location reference, {hf_uuid} doesn't match any HealthFacility."
            )
    ...
    def _family_reference_identifier_value(cls, family, reference_type):
        ...
        raise NotImplementedError(
            f"Reference type {reference_type} not implemented for family"
        )
    ...
    def _validate_fhir_photo(cls, fhir_patient):
        if not fhir_patient.photo or len(fhir_patient.photo) == 0:
            raise FHIRException(_("FHIR Patient without photo data."))
        else:
    ...
    def _validate_imis_identifier_code(cls, imis_insuree):
        if not imis_insuree.chf_id:
            raise FHIRException(
                ...
                % {"insuree_uuid": imis_insuree.uuid}
            )
    ...
    def _validate_fhir_family_home_slice(cls, fhir_patient):
        ...
        for address in fhir_patient.address:
            if address.use == "home":
                is_home_use = True
        if is_home_use is False:
            raise FHIRException(_("Patient without family address"))

    @classmethod
    def _validate_fhir_patient_human_name(cls, fhir_patient):
        if not fhir_patient.name:
            raise FHIRException(_("Missing fhir patient attribute: name"))
        for name in fhir_patient.name:
            if not name.family or not name.given:
                raise FHIRException(
                    _(
                        "Missing obligatory fields for fhir patient name: family or given"
                    )
                )
    ...
    def _validate_imis_insuree_human_name(cls, imis_insuree):
        if not imis_insuree.last_name or not imis_insuree.other_names:
            raise FHIRException(_("Missing patient family name or given name"))
    ...
    def __add_insuree_location(cls, imis_insuree, address):
        ...
        for ext in address.extension:
            if "StructureDefinition/address-location-reference" in ext.url:
                location_uuid = LocationConverter.get_resource_id_from_reference(
                    ext.valueReference
                )
                try:
                    ...
                    raise FHIRException(
                        f"Invalid location reference, {location_uuid} doesn't match any location."
                    )
    ...
    @classmethod
    def __build_address_of_use(
        cls, address_location: Location, use: str, location_text: str, reference_type
    ):
    ...
    @classmethod
    def __build_base_physical_address(
        cls, imis_village_location, reference_type
    ) -> Address:
        return Address(
            **{
                "type": "physical",
                "state": cls.__state_name_from_physical_location(imis_village_location),
                "district": cls.__district_name_from_physical_location(
                    imis_village_location
                ),
                "city": cls.__village_name_from_physical_location(
                    imis_village_location
                ),
                "extension": [
    ...
    @classmethod
    def __build_location_reference_extension(
        cls, insuree_family_location, reference_type
    ):
        extension = Extension.construct()
    ...
    def __build_imis_family_address(cls, fhir_patient):
        ...
        return (
            cls.__get_location_from_address(family_address) if family_address else None
        )

    @classmethod
    def __get_location_from_address(cls, fhir_patient_address):
        return LocationConverter.get_location_from_address(
            LocationConverter, fhir_patient_address
        )
    ...
    def __get_family_from_fhir_patient_extension(cls, fhir_patient):
        ...
        try:
            ...
            raise FHIRException(
                f"Invalid location reference, {e} doesn't match any location."
            )
    ...
    def __get_family_uuid_from_fhir_patient(cls, fhir_patient):
        return next(
            (
                ...
                if "/StructureDefinition/patient-group-reference" in ext.url
            ),
            None,
        )
