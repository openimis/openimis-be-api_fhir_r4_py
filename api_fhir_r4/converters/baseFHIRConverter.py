...
class BaseFHIRConverter(ABC):
    ...
    @classmethod
    def build_fhir_pk(
        cls, fhir_obj, resource: Union[str, Model], reference_type: str = None
    ):
    ...
    def get_fhir_extension_by_url(cls, extensions, url):
        return next(
            iter([extension for extension in extensions if extension.url == url]), None
        )
    ...
    def get_use_context_by_code(cls, use_context, code):
        return next(
            iter([entry for entry in use_context if entry.code.code == code]), None
        )
    ...
    @classmethod
    def build_fhir_codeable_concept_extension(
        cls, codeable_concept: CodeableConcept, url
    ):
        extension = Extension.construct()
        extension.url = url
    ...
    def build_imis_identifier(cls, imis_obj, fhir_obj, errors):
        ...
        code = cls.get_fhir_identifier_by_code(
            fhir_obj.identifier, R4IdentifierConfig.get_fhir_generic_type_code()
        )
        uuid_str = cls.get_fhir_identifier_by_code(
            fhir_obj.identifier, R4IdentifierConfig.get_fhir_uuid_type_code()
        )
        id_str = None
        if not history_model:
            id_str = cls.get_fhir_identifier_by_code(
                fhir_obj.identifier, R4IdentifierConfig.get_fhir_acsn_type_code()
            )
            if id_str:
                imis_obj.id = id_str
        if uuid_str:
