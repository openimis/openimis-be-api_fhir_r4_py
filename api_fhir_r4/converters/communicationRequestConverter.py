...
class CommunicationRequestConverter(BaseFHIRConverter, ReferenceConverterMixin):
    ...
    def build_fhir_status_reason(cls, fhir_communication_request, imis_claim):
        ...
        system = (
            f"{GeneralConfiguration.get_system_base_url()}CodeSystem/feedback-status"
        )
        fhir_communication_request.statusReason = cls.build_codeable_concept(
            system=system, code=imis_claim.feedback_status, display=display
        )
    ...
    @classmethod
    def build_fhir_recipient(
        cls, fhir_communication_request, imis_claim, reference_type
    ):
