...
class LocationSiteConverter(BaseFHIRConverter, ReferenceConverterMixin):
    ...
    @classmethod
    def to_fhir_obj(
        cls, imis_hf, reference_type=ReferenceConverterMixin.UUID_REFERENCE_TYPE
    ):
    ...
    def build_imis_hf_identiftier(cls, imis_hf, fhir_location, errors):
        value = cls.get_fhir_identifier_by_code(
            fhir_location.identifier, R4IdentifierConfig.get_fhir_facility_id_type()
        )
        if value:
            imis_hf.code = value
        cls.valid_condition(
            imis_hf.code is None, gettext("Missing location code"), errors
        )
    ...
    def build_imis_hf_name(cls, imis_hf, fhir_location, errors):
        ...
        if not cls.valid_condition(
            name is None, gettext("Missing location `name` attribute"), errors
        ):
            imis_hf.name = name
    ...
    def build_fhir_physical_type(cls, fhir_location, imis_hf):
        ...
        text = "site"
        fhir_location.physicalType = cls.build_codeable_concept(
            code, R4LocationConfig.get_fhir_location_physical_type_system(), text=text
        )
    ...
    def build_fhir_type(cls, fhir_location, imis_hf):
        ...
        fhir_location.type = [
            cls.build_codeable_concept(
                code, R4LocationConfig.get_fhir_location_site_type_system(), text=text
            )
        ]
    ...
    def build_imis_hf_level(cls, imis_hf, fhir_location, errors):
        ...
        cls.valid_condition(
            imis_hf.level is None, gettext("Missing location level"), errors
        )

    @classmethod
    def build_fhir_part_of(cls, fhir_location, imis_hf, reference_type):
        if imis_hf.location is not None:
            fhir_location.partOf = LocationSiteConverter.build_fhir_resource_reference(
                imis_hf.location, "Location", imis_hf.location.code, reference_type
            )
    ...
    def build_imis_parent_location_id(cls, imis_hf, fhir_location, errors):
        if fhir_location.partOf:
            ...
            if not cls.valid_condition(
                parent_id is None,
                gettext("Missing location `parent id` attribute"),
                errors,
            ):
