...
from api_fhir_r4.mapping.activityDefinitionMapping import (
    ServiceTypeMapping,
    UseContextMapping,
    VenueMapping,
    WorkflowMapping,
...
class ActivityDefinitionConverter(BaseFHIRConverter, ReferenceConverterMixin):
    ...
    def to_fhir_obj(
        ...
        cls.build_fhir_pk(
            fhir_activity_definition, imis_activity_definition, reference_type
        )
        ...
        cls.build_fhir_activity_definition_extension(
            fhir_activity_definition, imis_activity_definition
        )
    ...
    def to_imis_obj(cls, fhir_activity_definition, audit_user_id):
        ...
        cls.build_imis_identifier(
            imis_activity_definition, fhir_activity_definition, errors
        )
        cls.build_imis_validity_from(
            imis_activity_definition, fhir_activity_definition, errors
        )
        cls.build_imis_price(imis_activity_definition, fhir_activity_definition, errors)
        cls.build_imis_frequency(
            imis_activity_definition, fhir_activity_definition, errors
        )
        cls.build_imis_serv_code(
            imis_activity_definition, fhir_activity_definition, errors
        )
        cls.build_imis_serv_name(
            imis_activity_definition, fhir_activity_definition, errors
        )
        cls.build_imis_serv_type(
            imis_activity_definition, fhir_activity_definition, errors
        )
        cls.build_imis_serv_pat_cat(
            imis_activity_definition, fhir_activity_definition, errors
        )
        cls.build_imis_serv_category(
            imis_activity_definition, fhir_activity_definition, errors
        )
        cls.build_imis_serv_care_type(
            imis_activity_definition, fhir_activity_definition, errors
        )
    ...
    def get_imis_obj_by_fhir_reference(cls, reference, errors=None):
        return DbManagerUtils.get_object_or_none(
            Service, **cls.get_database_query_id_parameteres_from_reference(reference)
        )
    ...
    @classmethod
    def build_imis_price(
        cls, imis_activity_definition, fhir_activity_definition, errors
    ):
        ...
        extension = cls.get_fhir_extension_by_url(
            fhir_activity_definition.extension, extension_base_url
        )
    ...
    def build_imis_frequency(
        ...
        try:
            ...
            errors.append(
                gettext("Invalid activity definition `timingTiming` attribute")
            )
        finally:
            imis_activity_definition.frequency = value

    @classmethod
    def build_imis_identifier(
        cls, imis_activity_definition, fhir_activity_definition, errors
    ):
        value = cls.get_fhir_identifier_by_code(
            fhir_activity_definition.identifier,
        ...
        cls.valid_condition(
            not imis_activity_definition.code,
            gettext("Missing the service code"),
            errors,
        )
    ...
    def build_fhir_date(cls, fhir_activity_definition, imis_activity_definition):
        fhir_activity_definition.date = (
            imis_activity_definition.validity_from.isoformat()
        )

    @classmethod
    def build_imis_validity_from(
        cls, imis_activity_definition, fhir_activity_definition, errors
    ):
        validity_from = fhir_activity_definition.date
        if not cls.valid_condition(
            not validity_from,
            gettext("Missing activity definition `validity from` attribute"),
            errors,
        ):
            imis_activity_definition.validity_from = TimeUtils.str_iso_to_date(
                validity_from
            )
    ...
    @classmethod
    def build_imis_serv_code(
        cls, imis_activity_definition, fhir_activity_definition, errors
    ):
        serv_code = fhir_activity_definition.name
        if not cls.valid_condition(
            not serv_code,
            gettext("Missing activity definition `serv code` attribute"),
            errors,
        ):
    ...
    @classmethod
    def build_imis_serv_name(
        cls, imis_activity_definition, fhir_activity_definition, errors
    ):
        serv_name = fhir_activity_definition.title
        if not cls.valid_condition(
            not serv_name,
            gettext("Missing activity definition `serv name` attribute"),
            errors,
        ):
            imis_activity_definition.name = serv_name

    @classmethod
    def build_imis_serv_pat_cat(
        cls, imis_activity_definition, fhir_activity_definition, errors
    ):
        ...
        try:
            for use_context_code in use_context_codes:
                use_context = cls.get_use_context_by_code(
                    fhir_activity_definition.useContext, use_context_code
                )
                if use_context and len(use_context.valueCodeableConcept.coding) > 0:
                    for coding in use_context.valueCodeableConcept.coding:
                        pat_cat_flag = (
                            pat_cat_flag
                            | PatientCategoryMapping.imis_patient_category_flags[
                                coding.code
                            ]
                        )
            ...
            errors.append(
                gettext(
                    "Invalid activity definition `use context - age/gender` attribute"
                )
            )
        finally:
    ...
    @classmethod
    def build_imis_serv_category(
        cls, imis_activity_definition, fhir_activity_definition, errors
    ):
        use_context = cls.get_use_context_by_code(
            fhir_activity_definition.useContext, "workflow"
        )
        ...
        try:
            ...
            errors.append(
                gettext(
                    "Invalid activity definition `use context - workflow` attribute"
                )
            )
        finally:
    ...
    @classmethod
    def build_imis_serv_care_type(
        cls, imis_activity_definition, fhir_activity_definition, errors
    ):
        use_context = cls.get_use_context_by_code(
            fhir_activity_definition.useContext, "venue"
        )
        care_type = None
        try:
            if not use_context or len(use_context.valueCodeableConcept.coding) == 0:
                errors.append(
                    gettext(
                        "Missing activity definition `use context - venue` attribute"
                    )
                )
            else:
                for coding in use_context.valueCodeableConcept.coding:
            ...
            errors.append(
                gettext("Invalid activity definition `use context - venue` attribute")
            )
        finally:
            imis_activity_definition.care_type = care_type

    @classmethod
    def build_imis_serv_type(
        cls, imis_activity_definition, fhir_activity_definition, errors
    ):
        service_type = None
        try:
            if hasattr(fhir_activity_definition, "topic"):
                service_type = cls.get_first_coding_from_codeable_concept(
                    fhir_activity_definition.topic[0]
                ).code
                if (
                    service_type
                    not in ServiceTypeMapping.fhir_service_type_coding.keys()
                ):
                    raise AttributeError
        except AttributeError:
    ...
    @classmethod
    def build_fhir_activity_definition_extension(
        cls, fhir_activity_definition, imis_activity_definition
    ):
    ...
    def build_fhir_topic(cls, fhir_activity_definition, imis_activity_definition):
        topic = [
            cls.build_codeable_concept_from_coding(
                cls.build_fhir_topic_coding(imis_activity_definition.type)
            )
        ]
    ...
    def build_fhir_use_context(cls, fhir_activity_definition, imis_activity_definition):
        ...
        if (
            imis_activity_definition.category
            and not imis_activity_definition.category.isspace()
        ):
            usage.append(
                cls.build_fhir_workflow_usage_context(imis_activity_definition)
            )
        ...
        usage = [
            ...
            if len(usage_item.valueCodeableConcept.coding) > 0
        ]
    ...
    def build_fhir_gender_usage_context(cls, imis_activity_definition):
        return cls.build_fhir_usage_context(
            cls.build_fhir_usage_context_coding("gender"),
    ...
    def build_fhir_age_usage_context(cls, imis_activity_definition):
        return cls.build_fhir_usage_context(
            cls.build_fhir_usage_context_coding("age"),
    ...
    def build_fhir_venue_usage_context(cls, imis_activity_definition):
        return cls.build_fhir_usage_context(
            cls.build_fhir_usage_context_coding("venue"),
    ...
    def build_fhir_workflow_usage_context(cls, imis_activity_definition):
        return cls.build_fhir_usage_context(
            cls.build_fhir_usage_context_coding("workflow"),
    ...
    def build_fhir_workflow(cls, imis_activity_definition):
        ...
        codeable_concept.coding.append(
            cls.build_fhir_workflow_coding(imis_activity_definition.category)
        )
    ...
    def build_fhir_venue(cls, imis_activity_definition):
        ...
        ambulatory_venue = imis_activity_definition.care_type in [
            Service.CARE_TYPE_OUT_PATIENT,
            Service.CARE_TYPE_BOTH,
        ]
        if ambulatory_venue:
            ambulatory_coding = cls.build_fhir_venue_coding(
                Service.CARE_TYPE_OUT_PATIENT
            )
            codeable_concept.coding.append(ambulatory_coding)

        imp_venue = imis_activity_definition.care_type in [
        ...
        codeable_concept.text = " or ".join(
            [coding.display for coding in codeable_concept.coding]
        )
    ...
    def build_fhir_usage_context_coding(cls, usage_context):
        return cls.build_fhir_mapped_coding(
            UseContextMapping.fhir_use_context_coding[usage_context]
        )

    @classmethod
    def build_fhir_patient_category_coding(cls, category):
        return cls.build_fhir_mapped_coding(
            PatientCategoryMapping.fhir_patient_category_coding[category]
        )
    ...
    def build_fhir_workflow_coding(cls, workflow):
        return cls.build_fhir_mapped_coding(
            WorkflowMapping.fhir_workflow_coding[workflow]
        )

    @classmethod
    def build_fhir_topic_coding(cls, topic):
        return cls.build_fhir_mapped_coding(
            ServiceTypeMapping.fhir_service_type_coding[topic]
        )
    ...
    @classmethod
    def build_imis_level(
        cls, imis_service: Service, fhir_service: ActivityDefinition, errors: list
    ):
        level_ext = next(
            (
                ext
                for ext in fhir_service.extension
                if ext.url.lower().endswith("activity-definition-level")
            ),
            None,
        )
