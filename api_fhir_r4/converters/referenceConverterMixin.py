...
class ReferenceConverterMixin(object):
    ...
    def get_fhir_resource_type(cls):
        raise NotImplementedError(
            "`get_fhir_resource_type()` must be implemented."
        )  # pragma: no cover

    @classmethod
    def get_imis_obj_by_fhir_reference(cls, reference, errors=None):
        raise NotImplementedError(
            "`get_imis_object_by_fhir_reference()` must be implemented."
        )  # pragma: no cover

    @classmethod
    def build_fhir_resource_reference(
        cls, obj, type=None, display=None, reference_type=UUID_REFERENCE_TYPE
    ):
        if obj:
    ...
    @classmethod
    def get_database_query_id_parameteres_from_reference(
        cls, reference, code_keyword_name="code"
    ):
    ...
    def _get_reference_type(cls, reference_type):
        if (
            ...
            or reference_type.coding is None
            or reference_type.coding[0] is None
            or reference_type.coding[0].code is None
        ):
    ...
    @classmethod
    def _get_type_and_id_from_reference(
        ...
        if reference:
            ...
            if (
                reference.reference
                and isinstance(reference.reference, str)
                and "/" in reference.reference
            ):
                path, resource_id = reference.reference.rsplit("/", 1)
            elif (
                isinstance(reference.type, str)
    ...
    def __get_imis_object_id_as_string(cls, obj, reference_type):
        if reference_type == cls.UUID_REFERENCE_TYPE:
            ...
            raise FHIRRequestProcessException(
                [f"Could not create reference for reference type {reference_type}"]
            )
