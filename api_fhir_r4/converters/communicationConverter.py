...
class CommunicationConverter(BaseFHIRConverter, ReferenceConverterMixin):
    ...
    @classmethod
    def to_fhir_obj(
        cls, imis_feedback, reference_type=ReferenceConverterMixin.UUID_REFERENCE_TYPE
    ):
        fhir_communication = {}
    ...
    def to_imis_obj(cls, fhir_communication, audit_user_id):
        ...
        cls.build_imis_subject(
            imis_feedback, fhir_communication, errors, audit_user_id=audit_user_id
        )
    ...
    def build_imis_about(cls, imis_feedback, fhir_communication, errors):
        ...
        try:
            imis_feedback.claim = Claim.objects.get(
                uuid=claim_uuid, validity_to__isnull=True
            )
        except Exception:
            raise FHIRException(_("Claim does not exist"))
    ...
    def build_imis_care_rendered(cls, imis_feedback, fhir_content_string):
        cls._should_be_yes_or_no(
            Config.get_fhir_care_rendered_code(), fhir_content_string
        )
    ...
    def build_imis_payment_asked(cls, imis_feedback, fhir_content_string):
        cls._should_be_yes_or_no(
            Config.get_fhir_payment_asked_code(), fhir_content_string
        )
    ...
    def build_imis_drug_prescribed(cls, imis_feedback, fhir_content_string):
        cls._should_be_yes_or_no(
            Config.get_fhir_drug_prescribed_code(), fhir_content_string
        )
    ...
    def build_imis_drug_received(cls, imis_feedback, fhir_content_string):
        cls._should_be_yes_or_no(
            Config.get_fhir_drug_received_code(), fhir_content_string
        )
    ...
    def _should_be_yes_or_no(cls, code, content):
        if content not in ["yes", "no"]:
            raise ValidationError(
                f"Value for '{code}' must be either 'yes' or 'no' but is '{content}'"
            )
    ...
    def get_imis_obj_by_fhir_reference(cls, reference, errors=None):
        return DbManagerUtils.get_object_or_none(
            Feedback, **cls.get_database_query_id_parameteres_from_reference(reference)
        )
    ...
    def _validate_fhir_about(cls, fhir_communication):
        if not fhir_communication.about:
            raise FHIRException(_("about is required"))

    @classmethod
    def build_imis_subject(
        cls, imis_feedback, fhir_communication, errors, audit_user_id
    ):
        if not fhir_communication.subject:
            raise FHIRException(_("subject is required"))
        else:
            ...
            cls.valid_condition(
                not imis_feedback.claim.insuree,
                _("Missing or invalid `subject` reference"),
                errors,
            )
    ...
    def _validate_fhir_status(cls, fhir_communication):
        if not fhir_communication.status:
            raise FHIRException(_("status is required field"))
        else:
            if not fhir_communication.status == "completed":
                raise FHIRException(_("status value must be = completed"))

    @classmethod
    def _validate_fhir_payload(cls, fhir_communication):
        if not fhir_communication.payload:
            raise FHIRException(_("payload is required"))
        else:
            if len(fhir_communication.payload) != 5:
                raise FHIRException(_("payload must have 5 elements"))
            else:
                required_payload = [
                    "CareRendered",
                    "PaymentAsked",
                ...
                for payload in fhir_communication.payload:
                    extension_values.append(
                        cls.get_code_from_extension_codeable_concept(
                            payload.extension[0]
                        )
                    )
                ...
                if required_payload != extension_values:
                    raise FHIRException(
                        _(
                            "payload must support all five elemets:"
