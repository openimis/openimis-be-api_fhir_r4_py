...
from api_fhir_r4.subscriptions.notificationClient import (
    SubscriberNotificationOutput,
)
from api_fhir_r4.subscriptions.notificationManager import (
...
class TestSubscriptionNotificationManager(CommunicationTestMixin, LogInMixin, TestCase):
    ...
    EXPECTED_HEADER_1 = {
        ...
        "test-header": "123",
        "Authentication": "Bearer ABF13816",
    }
    ...
    EXPECTED_HEADER_2 = {
        ...
        "accept": "application/json",
        "test-header": "123",
        "Authentication": "Bearer 61831FAB",
    }
    ...
    def test_sending_claim_notification(self):
        ...
        mocked_client.propagate_notifications.return_value = [
            ...
            SubscriberNotificationOutput(
                self._test_subscriptions[1],
                False,
                {"ServerError": "Endpoint Unavailable"},
            ),
        ]
        test_manger = RestSubscriptionNotificationManager(
            fhir_converter=self._test_converter, client=mocked_client
        )
        test_manger.notify_subscribers_with_resource(
            self._test_resource, self._test_subscriptions
        )
    ...
    def _create_test_subscriptions(self):
        _valid_subscription = [
            self._create_valid(self.TEST_HEADERS_1),
            self._create_valid(self.TEST_HEADERS_2),
        ]
    ...
    def _create_valid(self, headers):
        sub = Subscription(
            status=1,
            channel=1,
            endpoint="http://test-subscription-endpoint.io/post_uri/",
            headers=headers,
            expiring=datetime.datetime.now() + datetime.timedelta(days=10),
        )
    ...
    def _assert_mock_call(self, mocked_client):
        mocked_client.propagate_notifications.assert_called_with(
            self._test_converter.to_fhir_obj(
                self._test_resource, ReferenceConverterMixin.UUID_REFERENCE_TYPE
            ).dict(),
            self._test_subscriptions,
        )

    def _assert_saved_result(self):
        success = SubscriptionNotificationResult.objects.subscriber_notifications(
            self._test_subscriptions[0]
        )
        failure = SubscriptionNotificationResult.objects.subscriber_notifications(
            self._test_subscriptions[1]
        )
