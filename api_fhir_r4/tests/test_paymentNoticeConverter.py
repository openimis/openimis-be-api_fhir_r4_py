...
class PaymentNoticeConverterTestCase(
    PaymentNoticeTestMixin, LogInMixin, ConvertJsonToFhirTestMixin
):
    ...
    def test_to_imis_obj(self):
        ...
        if "reconciliation" in imis_payment.json_ext:
            imis_payment_detail.reconcilation_id = imis_payment.json_ext[
                "reconciliation"
            ]["id"]
        imis_payment_detail.save(username=user.username)
