...
class GroupAPITests(
    ...
    def update_payload_no_such_chf_id(self, data):
        for member in data["member"]:
            member["entity"][
                "reference"
            ] = f"Patient/{self._TEST_INSUREE_CHFID_NOT_EXIST}"
        return data
    ...
    def test_post_should_raise_error_no_such_chf_id(self):
        self.login()
        self.create_dependencies()
        modified_payload = self.update_payload_no_such_chf_id(
            data=self._test_request_data
        )
        response = self.client.post(self.base_url, data=modified_payload, format="json")
    ...
    def test_post_should_raise_error_no_chf_id_in_payload(self):
        self.login()
        self.create_dependencies()
        modified_payload = self.update_payload_remove_chf_id_from_it(
            data=self._test_request_data
        )
        response = self.client.post(self.base_url, data=modified_payload, format="json")
