...
from api_fhir_r4.tests import FhirApiReadTestMixin
...
class ContractAPITests(GenericFhirAPITestMixin, FhirApiReadTestMixin, APITestCase, LogInMixin, ABC):
    ...
    def test_post_patient_subject_reference(self):
        ...
        response = self.client.post(
            self.base_url, data=test_request_data, format="json", **headers
        )
    ...
    def _assert_contract_created(self, response):
        ...
        self.assertTrue(
            Policy.objects.filter(
                uuid__iexact=policy_id, validity_to__isnull=True
            ).exists(),
    ...
    def _assert_insuree_family_created_in_process(self, response):
        ...
        self.assertEqual(
            insuree.current_village,
            family_policy.location,
            "Family created from insuree object should share location their location.",
        )

        self.assertEqual(
            family_policy.head_insuree,
            insuree,
    ...
    def test_simple_list_page_2(self):
        ...
        response = self.client.get(
            self.base_url + "?page-offset=2&page-size=1", format="json", **headers
        )
        self.assertEqual(
            response.status_code, status.HTTP_200_OK, json.loads(response.content)
        )
        self.assertIsNotNone(response.content)
