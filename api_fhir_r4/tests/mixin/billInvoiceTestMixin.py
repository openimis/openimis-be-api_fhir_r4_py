...
class BillInvoiceTestMixin(GenericTestMixin, FhirConverterTestMixin):
    ...
    def verify_fhir_instance(self, fhir_obj):
        ...
        self.verify_fhir_identifier(
            fhir_obj, R4IdentifierConfig.get_fhir_uuid_type_code(), self._TEST_BILL_UUID
        )
        self.verify_fhir_identifier(
            fhir_obj,
            R4IdentifierConfig.get_fhir_generic_type_code(),
            self._TEST_BILL_CODE,
        )
        self.verify_fhir_coding_exists(
            fhir_obj.type.coding, self._TEST_BILL_SUBJECT_TYPE_CODING
        )
        self.assertTrue(
            self._TEST_BILL_THIRD_PARTY_UUID in fhir_obj.recipient.reference
        )
    ...
    def verify_price_component(
        ...
        self.assertEqual(
            price_component.extension[0].valueMoney.currency, self._TEST_BILL_CURRENCY
        )
        self.assertEqual(
