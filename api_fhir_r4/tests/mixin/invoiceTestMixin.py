...
class InvoiceTestMixin(GenericTestMixin, FhirConverterTestMixin):
    ...
    def create_test_imis_instance(self):
        ...
        self._TEST_INVOICE_SUBJECT_TYPE = ContentType.objects.get(
            model__iexact="Family"
        )
        self._TEST_LINE_ITEM_CHARGE_ITEM = ContentType.objects.get(
            model__iexact="Policy"
        )
    ...
    def verify_fhir_instance(self, fhir_obj):
        if not (
            hasattr(self, "_TEST_INVOICE_SUBJECT_TYPE")
            and hasattr(self, "_TEST_LINE_ITEM_CHARGE_ITEM")
        ):
        ...
        self.verify_fhir_identifier(
            ...
            self._TEST_INVOICE_CODE,
        )
        self.verify_fhir_coding_exists(
            fhir_obj.type.coding, self._TEST_INVOICE_SUBJECT_TYPE_CODING
        )
        self.assertTrue(
            self._TEST_INVOICE_THIRD_PARTY_UUID in fhir_obj.recipient.reference
        )
        ...
        for price_component in fhir_obj.lineItem[0].priceComponent:
            if price_component.type == self._TEST_LINE_ITEM_BASE_PRICE_COMPONENT_TYPE:
                self.verify_price_component(
                    price_component,
                    self._TEST_LiNE_ITEM_QUANTITY,
                ...
                self.verify_price_component(
                    ...
                    -self._TEST_LINE_ITEM_UNIT_PRICE
                    * self._TEST_LiNE_ITEM_QUANTITY
                    * (self._TEST_LINE_ITEM_DISCOUNT),
                    self._TEST_LINE_ITEM_DISCOUNT_COMPONENT_TYPE,
                )
            elif (
                price_component.type
                == self._TEST_LINE_ITEM_DEDUCTION_PRICE_COMPONENT_TYPE
            ):
                self.verify_price_component(
                    price_component,
                    self._TEST_LINE_ITEM_DEDUCTION_FACTOR,
                    -self._TEST_LINE_ITEM_DEDUCTION,
                    self._TEST_LINE_ITEM_DEDUCTION_PRICE_COMPONENT_TYPE,
                )

    def verify_price_component(
        self, price_component, expected_factor, expected_amount, expected_type
    ):
        self.assertGreater(len(price_component.extension), 0)
        self.assertEqual(
            price_component.extension[0].valueMoney.currency,
            self._TEST_INVOICE_CURRENCY,
        )
        self.assertEqual(
            price_component.extension[0].valueMoney.value,
            self._TEST_LINE_ITEM_UNIT_PRICE,
        )
        if expected_type:
