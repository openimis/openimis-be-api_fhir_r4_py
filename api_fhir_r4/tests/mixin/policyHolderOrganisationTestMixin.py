...
class PolicyHolderOrganisationTestMixin(GenericTestMixin):
    ...
    def verify_fhir_instance(self, fhir_obj):
        ...
        actual_id_uuid = next(
            iter(
                ...
                if "UUID" == identifier.type.coding[0].code
            ),
            None,
        )
        ...
        actual_id_code = next(
            iter(
                identifier.value
                for identifier in fhir_obj.identifier
                if "Code" == identifier.type.coding[0].code
            ),
            None,
        )
        ...
        actual_municipality = next(
            iter(
                extension.valueString
                for extension in fhir_obj.address[0].extension
                if "municipality" in extension.url
            ),
            None,
        )
        self.assertEqual(self._TEST_MUNICIPALITY, actual_municipality)

        expected_contact_name = "%s %s" % (
            self._TEST_CONTACT_NAME,
            self._TEST_CONTACT_SURNAME,
        )
        ...
        actual_fax = next(
            iter(
                telecom.value for telecom in fhir_obj.telecom if "fax" == telecom.system
            ),
            None,
        )
    ...
    def create_test_imis_instance(self):
        location = DbManagerUtils.get_object_or_none(
            Location, name=self._TEST_VILLAGE, validity_to__isnull=True
        )
        ...
        return PolicyHolder(
            **{
                ...
                "locations": location,
                "contact_name": {
                    "name": self._TEST_CONTACT_NAME,
                    "surname": self._TEST_CONTACT_SURNAME,
                },
