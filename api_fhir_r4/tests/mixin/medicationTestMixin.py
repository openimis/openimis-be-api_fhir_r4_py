...
class MedicationTestMixin(GenericTestMixin):
    ...
    def verify_imis_instance(self, imis_obj):
        ...
        self.assertAlmostEqual(
            decimal.Decimal(self._TEST_MEDICATION_PRICE), imis_obj.price, places=2
        )
        ...
        self.assertEqual(
            self._TEST_MEDICATION_PATIENT_CATEGORY, imis_obj.patient_category
        )
    ...
    def create_test_fhir_instance(self):
        ...
        fhir_medication.form = MedicationConverter.build_codeable_concept(
            "package", text=self._TEST_MEDICATION_PACKAGE
        )
        ...
        unit_price = MedicationConverter.build_fhir_unit_price_extension(
            self._TEST_MEDICATION_PRICE
        )
        fhir_medication.extension = [unit_price]

        medication_type = MedicationConverter.build_fhir_medication_type_extension(
            self._TEST_MEDICATION_TYPE
        )
        fhir_medication.extension.append(medication_type)

        medication_frequency = (
            MedicationConverter.build_fhir_medication_frequency_extension(
                self._TEST_MEDICATION_FREQUENCY
            )
        )
        ...
        if self._TEST_MEDICATION_PATIENT_CATEGORY & male_flag:
            coding_male = MedicationConverter._build_fhir_coding(
        ...
        if self._TEST_MEDICATION_PATIENT_CATEGORY & female_flag:
            coding_female = MedicationConverter._build_fhir_coding(
                code="female", display="Female", system=administrative_system
            )
            MedicationConverter._append_to_list_codeable_concept(
                extension, coding_female
            )
        ...
        if self._TEST_MEDICATION_PATIENT_CATEGORY & adult_flag:
            coding_adult = MedicationConverter._build_fhir_coding(
                code="adult", display="Adult", system=age_type_system
            )
            MedicationConverter._append_to_list_codeable_concept(
                extension, coding_adult
            )
        if self._TEST_MEDICATION_PATIENT_CATEGORY & child_flag:
            coding_child = MedicationConverter._build_fhir_coding(
                code="child", display="Child", system=age_type_system
            )
            MedicationConverter._append_to_list_codeable_concept(
                extension, coding_child
            )
        extension.valueUsageContext.code = MedicationConverter._build_fhir_coding(
        ...
        if self._TEST_MEDICATION_CARE_TYPE is not None:
            if code != "B":
                ...
                coding_venue = MedicationConverter._build_fhir_coding(
                    code=code, display=display, system=venue_system
                )
                MedicationConverter._append_to_list_codeable_concept(
                    extension, coding_venue
                )
            else:
        ...
        fhir_medication.code = MedicationConverter.build_codeable_concept(
            self._TEST_MEDICATION_CODE, text=self._TEST_MEDICATION_NAME
        )
    ...
    def verify_fhir_instance(self, fhir_obj):
        ...
        self.assertAlmostEqual(
            decimal.Decimal(self._TEST_MEDICATION_PRICE),
            extension_unit_price.value,
            places=2,
        )
        ...
        self.assertEqual(
            self._TEST_MEDICATION_TYPE, extension_medication_type.coding[0].code
        )
        ...
        self.assertEqual(
            self._TEST_MEDICATION_FREQUENCY,
            extension_medication_frequency.repeat.period,
        )
        self.assertEqual(
            int(re.sub("[^0-9]", "", self._TEST_MEDICATION_PACKAGE)),
            fhir_obj.amount.numerator.value,
        )
        ...
        for identifier in fhir_obj.identifier:
            ...
            code = MedicationConverter.get_first_coding_from_codeable_concept(
                identifier.type
            ).code
