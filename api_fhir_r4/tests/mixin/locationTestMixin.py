...
class LocationTestMixin(GenericTestMixin):
    ...
    def setUp(self):
        ...
        if self.test_region is None:
            self.test_village = create_test_village(
                custom_props={
                    "code": self._TEST_VILLAGE_CODE,
                    "name": self._TEST_VILLAGE_NAME,
                }
            )
    ...
    def create_test_fhir_instance(self):
        ...
        system_definition = LocationConverter.PHYSICAL_TYPES.get(
            self._TEST_LOCATION_TYPE
        )
        fhir_location.physicalType = LocationConverter.build_codeable_concept(
            **system_definition
        )
    ...
    def verify_fhir_instance(self, fhir_obj):
        ...
        physical_type_code = LocationConverter.get_first_coding_from_codeable_concept(
            fhir_obj.physicalType
        ).code
