...
class EnrolmentOfficerPractitionerRoleTestMixin(GenericTestMixin):
    ...
    def setUp(self):
        ...
        self.test_substitution_officer = create_test_officer(
            custom_props={
        ...
        self._TEST_PRACTITIONER_REFERENCE = "Practitioner/" + str(
            self.test_officer.uuid
        )

        self.sub_str[self._TEST_SUBSTITUTION_OFFICER_UUID] = (
            self.test_substitution_officer.uuid
        )
        self.sub_str[self._TEST_OFFICER_UUID] = self.test_officer.uuid
        self.sub_str[self._TEST_VILLAGE_UUID] = self.test_village.uuid
        self.sub_str[self._TEST_VILLAGE_REFERENCE] = "Location/" + str(
            self.test_village.uuid
        )
        self._TEST_VILLAGE_REFERENCE = "Location/" + str(self.test_village.uuid)

    def verify_imis_instance(self, imis_obj):
        self.assertEqual(
            self.test_officer.substitution_officer.code,
            imis_obj.substitution_officer.code,
        )
    ...
    def verify_fhir_instance(self, fhir_obj):
        ...
        self.assertIn(
            str(self.test_substitution_officer.uuid),
