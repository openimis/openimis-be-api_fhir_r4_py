...
class ClaimAPIContainedTestBaseMixin:
    ...
    def setUp(self):
        ...
        self._test_request_data = load_and_replace_json(
            self._test_json_request_path, self.sub_str
        )

    def create_dependencies(self):

        region = create_test_location(
            "R", custom_props={"code": "R2", "name": "Tahida"}
        )
        district = create_test_location(
            "D", custom_props={"code": "R2D2", "name": "Vida", "parent": region}
        )
        ward = create_test_location(
            "M", custom_props={"code": "R2D2M1", "name": "Majhi", "parent": district}
        )
        create_test_location(
            "V", custom_props={"code": "R2D2M1V1", "name": "Radho", "parent": ward}
        )
        ...
        self.test_claim, self.test_insuree, policy, self.test_hf = (
            create_test_claim_context(
                ...
                items=[{"code": self._TEST_ITEM_CODE}],
                services=[{"code": self._TEST_SERVICE_CODE}],
            )
        )
        ...
        UserDistrict.objects.create(
            **{"user": self._TEST_USER._u, "location": district, "audit_user_id": -1}
        )
    ...
    def assert_response(self, response_json):
        ...
        for item in response_json["item"]:
            for adjudication in item["adjudication"]:
                self.assertEqual(
                    adjudication["category"]["coding"][0]["code"],
                    f"{Claim.STATUS_REJECTED}",
                )
                # 2 not in price list
    ...
    def _assert_unique_created(self, expected_uuid, django_model: Type[Model]):
        msg = (
            f"Contained resource should create unique object of type "
    ...
    def assert_contained(self, json_response):
        ...
        self.assertResourceExists(
            contained, "Patient", "AAAA08B5-6E85-470C-83EC-0EE9370F0000"
        )
        self.assertResourceExists(
            contained, "Organization", "AAAA5F9B-97C6-444B-AAD9-2FCCFD460000"
        )
        self.assertResourceExists(
            contained, "Group", "AAAA5232-9054-4D86-B4F2-0E9C4ADF0000"
        )
        self.assertResourceExists(
            contained, "Practitioner", "AAAA5229-DD11-4383-863C-E2FAD1B20000"
        )
        self.assertResourceExists(
            contained, "Medication", "AAAA76E2-DC28-4B48-8E29-3AC4ABEC0000"
        )
        self.assertResourceExists(
            contained, "ActivityDefinition", "AAAA29BA-3F4E-4E6F-B55C-23A488A10000"
        )
    ...
    def assertResourceExists(self, contained, resource_type, resource_id):
        x = [
            x
            for x in contained
            if x["resourceType"] == resource_type and x["id"] == resource_id
        ]
        self.assertEqual(len(x), 1)


class ClaimAPIContainedTests(
    ClaimAPIContainedTestBaseMixin, GenericFhirAPITestMixin, APITestCase
):
    base_url = GeneralConfiguration.get_base_url() + "Claim/"
    ...
    def test_post_should_create_correctly(self):
        response = self.client.post(
            ...
            data=get_connection_payload(self._TEST_DATA_USER),
            format="json",
        )
