...
class PaymentNoticeAPITests(
    ...
    def test_post_wrong_status(self):
        invoice = self.create_dependencies()
        headers = self.initialize_auth()
        modified_payload = self._update_payload_wrong_status(
        ...
        self.assertEqual(
            ...
            _(
                ...
                ",\nBad value in paymentStatus, "
                "should be either 'active', 'cancelled', draft', 'entered-in-error'"
            ),
        )
        Invoice.objects.filter(id=invoice.id).delete()
    ...
    def test_post_wrong_payment_status(self):
        invoice = self.create_dependencies()
        headers = self.initialize_auth()
        modified_payload = self._update_payload_wrong_payment_status(
            data=self._test_request_data
        )
        response = self.client.post(
            self.base_url, data=modified_payload, format="json", **headers
        ...
        self.assertEqual(
            self.get_response_details(response_json_no_name),
            _(
                "The request cannot be processed due to the following "
    ...
    def test_post_wrong_request_type(self):
        invoice = self.create_dependencies()
        headers = self.initialize_auth()
        modified_payload = self._update_payload_wrong_request_type(
            data=self._test_request_data
        )
        response = self.client.post(
            self.base_url, data=modified_payload, format="json", **headers
