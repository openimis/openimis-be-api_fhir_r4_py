...
class AuthorizationAPITests(GenericFhirAPITestMixin, APITestCase):
    ...
    def test_post_should_authorize_correctly(self):
        response = self.client.post(
            self.base_url + "login/", data=get_connection_payload(), format="json"
        )
    ...
    def test_post_should_raise_error_decode_token(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(
            self.get_response_details(response_json), _("INCORRECT_CREDENTIALS")
        )
    ...
    def test_post_should_raise_forbidden(self):
        _TEST_DATA_USER_NO_ROLE = {
            ...
            "language": "en",
            "roles": [create_test_role(perm_names=[], name="EmptyRole").id]
        }
        get_or_create_user_api(_TEST_DATA_USER_NO_ROLE)
        response = self.client.post(
            self.base_url + "login/",
            data=get_connection_payload(_TEST_DATA_USER_NO_ROLE),
            format="json",
        )
        response_json = response.json()
        token = response_json["token"]
