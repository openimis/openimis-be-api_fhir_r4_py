...
class InsurancePlanConverter(BaseFHIRConverter, ReferenceConverterMixin):
    ...
    def build_fhir_status(cls, fhir_insurance_plan, imis_product):
        ...
        status = "unknown"
        if imis_product.date_from and now < imis_product.date_from:
            status = "draft"
        elif imis_product.date_from and imis_product.date_to and now >= imis_product.date_from and now <= imis_product.date_to:
            status = "active"
        elif imis_product.date_to and now > imis_product.date_to:
            status = "retired"
    ...
    def build_imis_coverage(cls, imis_product, fhir_insurance_plan):
        if fhir_insurance_plan.coverage and len(fhir_insurance_plan.coverage) == 1:
    ...
    def build_fhir_plan(cls, fhir_insurance_plan, imis_product):
        currency = core.currency if hasattr(core, 'currency') else "EUR"
        ...
        if imis_product.premium_adult:
            plan.generalCost.append(
                cls.__build_fhir_general_cost("premiumAdult", _("Premium Adult"), system, currency, imis_product.premium_adult)
            )
        if imis_product.premium_child:
            plan.generalCost.append(
                cls.__build_fhir_general_cost("premiumChild", _("Premium Child"), system, currency, imis_product.premium_child)
            )
        if imis_product.registration_lump_sum:
            plan.generalCost.append(
                cls.__build_fhir_general_cost("registrationLumpsum", _("Registration Lumpsum"), system, currency, imis_product.registration_lump_sum)
            )
        if imis_product.registration_fee:
            plan.generalCost.append(
                cls.__build_fhir_general_cost("registrationFee", _("Registration Fee"), system, currency, imis_product.registration_fee)
            )
        if imis_product.general_assembly_lump_sum:
            plan.generalCost.append(
                cls.__build_fhir_general_cost("generalAssemblyLumpSum", _("General Assembly Lumpsum"), system, currency, imis_product.general_assembly_lump_sum)
            )
        if imis_product.general_assembly_fee:
            plan.generalCost.append(
                cls.__build_fhir_general_cost("generalAssemblyFee", _("General Assembly Fee"), system, currency, imis_product.general_assembly_fee)
            )
        fhir_insurance_plan.plan = [plan]

    @classmethod
    def build_imis_plan(cls, imis_product, fhir_insurance_plan):
        if fhir_insurance_plan.plan and len(fhir_insurance_plan.plan) == 1:
    ...
    def __build_fhir_general_cost(cls, code, display, system, currency, value):
        ...
        general_cost.type.coding[0].display = display
        return general_cost
    ...
    def build_fhir_extentions(cls, fhir_insurance_plan, imis_product):
        ...
        def build_extension(target_insurance_plan, target_product, value):
            ...
            if value == "conversion":
                ...
                reference_conversion.reference = f"InsurancePlan/{target_product.code}"
                extension.valueReference = cls.build_fhir_resource_reference(target_product, 'InsurancePlan')
                extension.valueReference.display = target_product.code
            elif value == "max-installments":
                extension.url = f"{GeneralConfiguration.get_system_base_url()}StructureDefinition/insurance-plan-{value}"
                extension.valueUnsignedInt = target_product.max_installments
            elif value == "start_cycle1":
                cls.__build_fhir_cycle(extension, value, target_product.start_cycle_1)
            elif value == "start_cycle2":
                cls.__build_fhir_cycle(extension, value, target_product.start_cycle_2)
            elif value == "start_cycle3":
                cls.__build_fhir_cycle(extension, value, target_product.start_cycle_3)
            elif value == "start_cycle4":
                cls.__build_fhir_cycle(extension, value, target_product.start_cycle_4)
            elif value == "administration-period":
                cls.__build_fhir_period_extension(extension, value, target_product.administration_period)
            elif value == "payment-grace-period":
                cls.__build_fhir_period_extension(extension, value, target_product.grace_period_enrolment)
            elif value == "renewal-grace-period":
                cls.__build_fhir_period_extension(extension, value, target_product.grace_period_renewal)
            elif value == "renewal-discount":
                cls.__build_fhir_discount_extension(extension, value, target_product.renewal_discount_perc, target_product.renewal_discount_period)
            elif value == "enrolment-discount":
                cls.__build_fhir_discount_extension(extension, value, target_product.enrolment_discount_perc, target_product.enrolment_discount_period)
            if type(target_insurance_plan.extension) is not list:
                target_insurance_plan.extension = [extension]
            else:
                target_insurance_plan.extension.append(extension)
    ...
    def build_imis_extentions(cls, imis_product, fhir_insurance_plan):
        ...
        for extension in fhir_insurance_plan.extension:
            if "conversion" in extension.url:
                ...
                imis_product.conversion_product = products.first() if products else None
    ...
    def _validate_fhir_insurance_plan_identifier_code(cls, fhir_insurance_plan_identifier_code):
        if not fhir_insurance_plan_identifier_code:
            raise FHIRException(_('InsurancePlan FHIR without code - this field is obligatory'))
