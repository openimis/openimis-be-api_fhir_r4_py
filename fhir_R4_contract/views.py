...
class ContractViewSet(
    ...
    """
    FHIR Contract Resource ViewSet
    Maps openIMIS Policy to FHIR Contract resource
    """
    lookup_field = 'uuid'
    ...
    def list(self, request, *args, **kwargs):
        queryset = self.get_queryset() \
            ...
            .prefetch_related(
                Prefetch('insuree_policies', queryset=InsureePolicy.objects.select_related('insuree'))
            )
        refDate = request.GET.get('refDate')
        refEndDate = request.GET.get('refEndDate')
        identifier = request.GET.get("identifier")
        if identifier:
            queryset = queryset.filter(uuid=identifier)
        else:
            ...
            if refEndDate != None:
                try:
                    ...
                    queryset = queryset.filter(validity_from__lte=datevar)
                except ValueError:
                    pass
